...
class ParserTest(unittest.TestCase):
    ...
    def test_msg_resp(self):
        msg = (
            b"\x00\x00\x00&\x00\x00\x00\x02\x13\x8c4\xcd\x01x~\x83"
    ...
    def test_error_resp(self):
        error_msg = (
            b"\x00\x00\x002\x00\x00\x00\x01E_BAD_TOPIC PUB topic "
...
class CommandEncoderTest(unittest.TestCase):
    ...
    def test_pub_command(self):
        command_raw = self.parser.encode_command(b"PUB", b"foo", data=b"test_msg")
    ...
    def test_mpub_command(self):
        command_raw = self.parser.encode_command(
            b"MPUB", b"topic", data=[b"foo", b"bar"]
        )
        required_command = (
        ...
        command_raw = self.parser.encode_command(b"MPUB", "topic", data=["foo", "bar"])
        self.assertEqual(command_raw, required_command)
